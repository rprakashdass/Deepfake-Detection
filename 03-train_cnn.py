...
# model.compile(optimizer = Adam(l=0.0001), loss='binary_crossentropy', metrics=['accuracy'])
...
# Assuming 0.5 as the threshold for classification
threshold = 0.5

# Label the predictions based on the threshold
test_results['Label'] = test_results['Prediction'] >= threshold

# Display the DataFrame
print(test_results)

# Load and display the image if it is a deepfake
if test_results['Label'].iloc[0]:  # Assuming there's only one image in the DataFrame
    import matplotlib.pyplot as plt
    from PIL import Image

    # Load and display the image
    image_path = "img/s.png"  # Assuming the image path is specified correctly
